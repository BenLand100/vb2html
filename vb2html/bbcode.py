...
VERBATIM_KWARGS = dict(swallow_trailing_newline=True, replace_links=False, replace_cosmetic=False, escape_html=True, render_embedded=False)

class BBCodeVB4:
    
    def __init__(self, prefix='/srl/', old_domain='villavu.com'):
        ...
        self.parser.add_simple_formatter('html','<pre><code>%(value)s</code></pre>', **VERBATIM_KWARGS)
        self.parser.add_simple_formatter('code','<pre><code>%(value)s</code></pre>', **VERBATIM_KWARGS)
        self.parser.add_simple_formatter('report','<pre><code>%(value)s</code></pre>', **VERBATIM_KWARGS)
        self.parser.add_simple_formatter('simba','<pre><code>%(value)s</code></pre>', **VERBATIM_KWARGS)
        self.parser.add_simple_formatter('scar','<pre><code>%(value)s</code></pre>', **VERBATIM_KWARGS)
        self.parser.add_simple_formatter('highlight','<pre><code>%(value)s</code></pre>', **VERBATIM_KWARGS)
    ...
    def clean_href(self, href):
        ...
        if self.old_domain in href:
            ...
            if self.old_domain+'/forum/forumdisplay.php?f=' in href:
                href = href.replace(self.old_domain+'/forum/forumdisplay.php?f=',f'{self.prefix}f/')
                href = href.replace('https://','').replace('http://','')
            if self.arch_re.match(href):
                href = self.arch_re.replace(f'{self.prefix}t/\1',href)
                href = href.replace('https://','').replace('http://','')
            if self.old_domain+'/forum/showthread.php?t=' in href:
                href = href.replace(self.old_domain+'/forum/showthread.php?t=',f'{self.prefix}t/')
                href = href.replace('https://','').replace('http://','')
        return href
        ...
        if 'size' in options:
            try:
                ...
                if size == 0:
                    ...
                    size = f'{int(round(100*math.sqrt(size))):0d}%'
                else:
                    size = f'{int(round(100/math.sqrt(-size))):0d}%'
